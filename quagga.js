(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.quagga) {
      window.wwload.quagga = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/quagga/dist/quagga.min","libs/webrtc-adapter/release/adapter"], function() {
        window.wwload.quagga = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.quagga === "wait") {
      setTimeout(function () {
   ...
   */
   function quaggajs($ele){

    var config = {};
    config.width = eval($ele.attr("data-width"));  //设置视频流(屏幕)宽度,宽度选项是单个条的宽度。例如375
    config.height = eval($ele.attr("data-height")); //设置视频流(屏幕)高度.例如640
    config.facingMode  = $ele.attr("data-facingmode");  //设置前置和后置摄像头.例如前置user ,后置environment
    //$ele.find(".canvanCode").click(function(){
      ...
      Quagga.init({
            inputStream : {
                ...
                type : "LiveStream",
                constraints: {
                width: config.width,  //像素
                height: config.height,
                facingMode: config.facingMode, //user  environment   设置前置和后置摄像头
            ...
            if (err) {
                console.log(err);
                //$.wwclass.helper.anijsTrigger($ele, "quaggaerror");
                return
            }
            ...
            Quagga.onDetected(function(data){
                ...
                var value = data.codeResult.code;
                console.log(value);
                $.wwclass.helper.updateProp($ele, "data-x-quaggaresult", data);
                $.wwclass.helper.updateProp($ele, "data-x-quaggavalue", value);
                //$.wwclass.helper.anijsTrigger($ele, "quaggasuccess");
                Quagga.stop();
            })
        });
